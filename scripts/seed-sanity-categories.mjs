...
async function mutateDocuments({projectId, dataset, token, apiVersion}, documents) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Sanity category seed failed with ${response.status} ${response.statusText}: ${errorText}`)
  }
...
async function main() {
  ...
  const documents = buildCategoryDocuments()
  ...
  console.log(`Seeded ${documents.length} category documents into ${config.projectId}/${config.dataset}.`)
  console.log(`Category slugs: ${categoryDefinitions.map((category) => category.slug).join(', ')}`)
  if (ids.length > 0) {
