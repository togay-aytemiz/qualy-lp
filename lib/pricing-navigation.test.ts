...
describe('pricing navigation links', () => {
  it('routes pricing links to localized pricing pages instead of home hash sections', () => {
    ...
    expect(navbarSource).toContain("const pricingPath = language === 'en' ? '/en/pricing' : '/pricing';");
    expect(navbarSource).not.toContain('href="#pricing"');

    expect(footerSource).toContain("const pricingPath = language === 'en' ? '/en/pricing' : '/pricing';");
  ...
  it('uses a home link for the navbar logo instead of scroll-only behavior', () => {
    ...
    expect(navbarSource).toContain('<a href={homePath}');
